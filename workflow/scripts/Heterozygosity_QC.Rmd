...
het.raw <- as_tibble(read.table(params$Path_HetFile, header = TRUE, check.names = FALSE, as.is = TRUE))
```
...
Figure \@ref(fig:plothet) displays the distrubution of heterozygosity in `r params$Sample`. Samples with excessive (Het > `r upper.het`) or deficient (Het < `r lower.het`) heterozygosity are colored red. Table \@ref(tab:het) displays samples that are to be excluded.
