...
## ---- Read in Data ----##
#TODO: Fix params$Pi_threshold being null
...
ggplot(dat_forplots, aes(x = PI_HAT)) +
  ...
  geom_vline(xintercept = snakemake@params[["pi_threshold"]], colour = 'red', linetype = 2)
...
The following plot (Fig. \@ref(fig:relplot)) shows the `r plot2Ydesc` by the proportion of loci where individules share zero alleles (Z0), where the proportion of IBD sharing is greater than 0.05. In family based studies, pairs are coloured by IBD relationship. Table \@ref(tab:ibdfail) displays the individules where the kinship coefficent was greather than `r snakemake@params[["pi_threshold"]]` in population based studies OR how were duplicates in family based studies.
...
if(params$Family == "F" || params$Family == F) {
  dat_forplots %>%
    ...
    ggplot(aes(colour = related)) +
      ...
      scale_colour_manual(name = sprintf("IBD Sharing > %f", snakemake@params[["pi_threshold"]]),
...
`r tabcap("ibdfail", sprintf("Samples with a proportion of IBD sharing > %e", snakemake@params[["pi_threshold"]]))`

```{r table ibd, echo=FALSE}
ibd_tab %>% disptab
```

To construct a sample of unrelated individuals, participants with the highest number of proportions of IBD sharing > `r snakemake@params[["pi_threshold"]]` can be iterativly removed.
