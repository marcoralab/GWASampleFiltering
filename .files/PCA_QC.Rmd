...
params:
   path.eigenvec: ~/Dropbox/Research/PostDoc/MitoWax/2_Derived_Data/GWAS_pres_processing/ADNI_1/merged_1000_adni_clust.eigenvec
   path.eigenval: ~/Dropbox/Research/PostDoc/MitoWax/2_Derived_Data/GWAS_pres_processing/ADNI_1/merged_1000_adni_clust.eigenval
   path.base_pops: ~/Documents/0_Data/1000genomes/20130606_g1k.ped.txt
   path.target_pops: /Users/sheaandrews/Dropbox/Research/PostDoc/MitoWax/2_Derived_Data/GWAS_pres_processing/ADNI_1/ADNI1_05_thined.fam
   path.out: ~/Dropbox/Research/PostDoc/MitoWax/2_Derived_Data/GWAS_pres_processing/ADNI_1/
   cohort: 'ADNI'
editor_options:
...
Prior to performing the PCA, the dataset should be LD pruned to eliminate a large degree of the redundency in the data and reduce the influce of potential chromsomal artifacts and related samples should be excluded. A PCA can be conducted with PLINK.
...
##---- Read in Data ----##
# PCA file from plink
pca.raw <- as.tibble(read_delim(params$path.eigenvec, delim = " ", col_names = F))

# population data file from 1000 genomes
base_pops.raw <- read_tsv(params$path.base_pops)

# population data from target set
target_pops.raw <- read_delim(params$path.target_pops, delim = " ", col_names = F)

# egien values
eigenval.raw <- read_table(params$path.eigenval, col_names = F)
```
...
target_pops <- target_pops.raw %>%
  select(X2) %>%
  rename(IID = X2) %>%
  mutate(Population = params$cohort, SuperPopulation = params$cohort, cohort = params$cohort, Population_2 = params$cohort)
...
pca$Population_2 <- factor(pca$Population, level = c(params$cohort, 'GBR', 'FIN', 'IBS', 'CEU', 'TSI', 'PUR', 'CLM', 'PEL', 'MXL', 'CHS', 'CDX', 'KHV', 'CHB', 'JPT', 'PJL', 'BEB', 'STU', 'ITU', 'GIH', 'GWD', 'ESN', 'MSL', 'YRI', 'LWK', 'ASW', 'ACB'))
...
```{r 3PC_interactive_SuperPops, echo=F, fig.width=7.5, fig.height=4.7, fig.align='center', fig.cap='Fig. 5: Tridimensional Plot of PC 1, 2 & 3 for Autosmoal Population Structure'}
##  Plot Superpopulations, PC1 + PC2 + PC3
plot_ly(pca, x = ~PC2, y = ~PC1, z = ~PC3, color = ~SuperPopulation, type = 'scatter3d', mode = 'markers', marker = list(size = 3),
        colors = c('black', "#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00"), hoverinfo = 'text', text = ~paste('</br> ID: ', IID,
...
## Population Outliers
Individules of non-European ancestry were identified by determining the mean and standard deviation of the 10 principal components scores for the EUR super population (Table 1). Participants in `r params$cohort` who were \pm 6 SD from the EUR population mean were determined to be of non-European ancestry (Table 2).
...
EurPca <- pca %>%
  ...
  summarize(mean = mean(eigenvalue), sd = sd(eigenvalue)) %>%
  mutate(lower = mean - sd*6, upper = mean + sd*6) %>%
  mutate(PC = factor(PC, levels = paste0('PC', 1:10))) %>%
  arrange(PC)

##  Print table for meand and SD
EurPca %>%
  knitr::kable(format = 'html', caption = 'Table 1: Mean and SD of PC in EUR population') %>%
...
Fig. 6 and Fig. 7 display the population structure of `r params$cohort` with EUR population outliers highlighted in red.
...
sample.pca %>%
  ...
  knitr::kable(format = 'html', caption = 'Table 2: EUR Population Outliers for each PC') %>%
  kable_styling(full_width = F, position = "center") %>%
