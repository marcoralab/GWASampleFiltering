...
pca_col <- pca %>%
  ...
  mutate(colour = ifelse(SuperPopulation == params$Sample, 'black', NA)) %>%
  mutate(colour = ifelse(SuperPopulation == 'AFR', "#E41A1C", colour)) %>%
  mutate(colour = ifelse(SuperPopulation == 'AMR', "#377EB8", colour)) %>%
  mutate(colour = ifelse(SuperPopulation == 'EAS', "#4DAF4A", colour)) %>%
  mutate(colour = ifelse(SuperPopulation == 'EUR', "#984EA3", colour)) %>%
  mutate(colour = ifelse(SuperPopulation == 'SAS', "#FF7F00", colour))

##  Plot Superpopulations, PC1 + PC2
ggplot(pca, aes(x = PC2, y = PC1, colour = SuperPopulation, shape = cohort)) +
  geom_point() + facet_zoom(xy = SuperPopulation == 'EUR') + scale_color_manual(values = pca_col$colour) +
...
```{r population outliers, message=F, echo=F}
## ---- Population Outliers ---- ##
...
cat('Figures 6 & 7 and Table 2 are omitted because there are no outliers.')
knitr::knit_exit()
```
