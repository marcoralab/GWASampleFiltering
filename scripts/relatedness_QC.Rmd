...
ggplot(dat.forplots, aes(x = PI_HAT)) +
  geom_histogram(binwidth = 0.01, fill = "#377EB8") + lims(x = (min(dat.forplots$PI_HAT) - 0.05):1) +
...
The following plot (Fig. \@ref(fig:relplot)) shows the `r plot2Ydesc` by the proportion of loci where individules share zero alleles (Z0), where the proportion of IBD sharing is greater than 0.05. In family based studies, pairs are coloured by IBD relationship.
...
if(params$Family & !no_relateds){
  ggplot(dat.inter, aes(colour = relationship)) +
  geom_point(aes_string(x = plot2Xcol, y = plot2Ycol)) + lims(x = 0:1, y = 0:1) +
  labs(x = 'IBD Z0', y = plot2Y) +
  coord_fixed() + theme_bw() + theme(legend.position="right")
} else {
  ggplot(dat.forplots, aes(colour = PI_HAT > Pi_threshold)) +
...
```{r early_exit, echo=F, results='asis', eval=no_relateds}
cat('Tables for cryptic relatedness are omitted because there are no outliers.')
knitr::knit_exit()
```

Table \@ref(tab:ibdfail) displays the individales where the kinship coefficent was greather than `r Pi_threshold` in population based studies OR how were duplicates in family based studies.
