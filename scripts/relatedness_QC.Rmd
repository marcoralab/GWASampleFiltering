...
The following plot (Fig. 2) shows the proportion loci sharing one allele IBD (Z1) by the proportion of loci where individules share zero alleles (Z0), where the kinship coefficent is greater than 0.05. In family based studies, pairs are coloured by IBD relationship. Table 2 displays the individules where the kinship coefficent was greather than `Pi_threshold` in population based studies OR how were duplicates in family based studies.
...
if(params$Family == F){
  ...
  scale_colour_manual(name = sprint("PI HAT > %d", Pi_threshold),
...
To construct a sample of unrelated individules, participants with the highest number of pairwise kinship coefficents > `Pi_threshold` can be iterativly removed.
