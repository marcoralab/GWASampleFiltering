...
geno <- read.plink(paste0('Data/', params$Sample, '.bed'), paste0('Data/', params$Sample, '.bim'), paste0('Data/', params$Sample, '.fam'))

# ---- readin plink .bim ---- ##
bim.raw <- read_tsv( paste0('Data/', params$Sample, '.bim'), col_names = F)
...
hwe.dat <- snpsum.col %>% 
  ...
  bind_cols(bim) %>% 
  select(-CHR, -CM, -A1, -A2) %>% 
  left_join(filter(hwe.raw, TEST %in% 'ALL'), by = c('RS' = 'SNP')) %>% 
