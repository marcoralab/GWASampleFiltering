...
coords_t <- snps %>% filter(hwe & !is.na(AA+AB+BB)) %>% select(AB, BB, AA)
coords_f <- snps %>% filter(!hwe & !is.na(AA+AB+BB)) %>% select(AB, BB, AA)
...
AddToTernary(points, coords_t, col=adjustcolor("blue", alpha.f = 0.3), pch=16, cex=0.3)
AddToTernary(points, coords_f, col=adjustcolor("red", alpha.f = 0.3), pch=16, cex=0.3)
